...
from models.basic_model_new import AE, HCCM
...
class Net(nn.Module):
    def __init__(self, args, config):
        ...
        self.diffusion_model = GaussianDiffusion(
            unet, image_size=64, objective="pred_x0", sampling_timesteps=25
        )
    ...
    def load_stage1(model, model_dir):
        checkpoint = utils.logging.load_checkpoint(
            os.path.join(model_dir, "stage1_weight.pth.tar"), "cuda"
        )
    ...
    def forward(self, x, y_gray=None, y=None):
        ...
        gt_gray_fea_down, raw_fea_down = (
            output["gt_gray_fea_down"],
            output["raw_fea_down"],
        )
        ...
        if self.training:
            ...
            pred_gt_gray_fea_down = self.diffusion_model(
                gt_gray_fea_down_norm, x_cond=raw_fea_down
            )
        else:
            pred_gt_gray_fea_down = self.diffusion_model.sample(
                batch_size=b, x_cond=raw_fea_down
            )
...
class ISPDiffuser(object):
    ...
    def train(self, DATASET):
        ...
        self.logs_path = os.path.join(self.config.data.ckpt_dir, "metrics_new.csv")
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y, y_gray, img_id) in enumerate(train_loader):
                ...
                x, y, y_gray = (
                    x.to(self.device),
                    y.to(self.device),
                    y_gray.to(self.device),
                )

                output = self.model(
                    x, y_gray=y_gray, y=y
                )  # FIX: keyword args to avoid positional swap bug

                diff_loss, feature_loss, content_loss, hist_loss, texture_loss = (
                    self.noise_estimation_loss(output, y)
                )
                loss = (
                    diff_loss
                    + feature_loss
                ...
                if self.step % 10 == 0:
                    print(
                        "step:{}, diff_loss:{:.5f} feature_loss:{:.5f} hist_loss:{:.5f} content_loss:{:.5f} time:{:.5f}".format(
                            self.step,
                            diff_loss.item(),
                            feature_loss.item(),
                            hist_loss.item(),
    ...
    def noise_estimation_loss(self, output, y):
        gt_gray_fea, recon_gt_gray_fea = (
            output["gt_gray_fea"],
            output["pred_gt_gray_fea"],
        )
    ...
    def sample_validation_patches(self, val_loader, step):
        ...
        with torch.no_grad():
            ...
            for i, (x, y, y_gray, img_id) in enumerate(val_loader):
                ...
                x = x.to(self.device)
                y_np_batch = (
                    y.permute(0, 2, 3, 1).detach().cpu().numpy()
                )

                # Single forward on full batch
                pred_x = self.model(x)["recon_gt_img"]
                recon_np_batch = (
                    pred_x.permute(0, 2, 3, 1).detach().cpu().numpy()
                )

                # Loop over samples in the batch (skimage/lpips are per-image)
                for j in range(b):
                    y_np = np.clip(y_np_batch[j] * 255.0, 0, 255.0).astype("uint8")
                    recon_img = (
                        np.clip(recon_np_batch[j] * 255.0, 0, 255.0).astype("uint8")
                    )

                    psnr_gt = peak_signal_noise_ratio(
                        recon_img, y_np, data_range=255
                    )
                    ...
                    lpips_gt = calc_lpips(recon_img, y_np, loss_fn_alex_v1)

                    psnr_adder(psnr_gt)
                    ssim_adder(ssim_gt)
                    lpips_adder(lpips_gt)
            print(
                "avg psnr: {}, avg ssim: {} lpips:{}".format(
                    psnr_adder.average(), ssim_adder.average(), lpips_adder.average()
                )
            )
